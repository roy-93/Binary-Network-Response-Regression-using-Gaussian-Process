...
V<-dim(A)[1] # number of regions(68)
N<-dim(A)[3] # number of subjects(114)

Y=FSIQ
Y_s = sort(unique(Y)) # 48 Unique scores
n = length(Y_s)
...
# This index means in which position of Y_s contains the similar value of Y.(Aninda)
...
###------ kappa_w ------###
kappa_w = 0.001
cov_W = matrix(0,nc=n,nr=n) #Squared Exponential Correlation Function(Aninda)
for (j in 1:n){
...
Z_samples = array(0, c(V,V,N_s)) # shared parameters(Aninda)(V=68,N_s=1000)
#-----------------------------------------------------------------------------#
# X[v,r,i,]=X_{v,r}^{(i)}
X_samples <- array(0, c(V,R,N,N_s)) # Subject specific latent coordinates(Aninda)
#-----------------------------------------------------------------------------#
# Shared behavior G
#G_samples = array(0, c(V,K,N_s)) # Coefficients common to all the subjects(Aninda)
#-----------------------------------------------------------------------------#
# W with W[i,V,r]= W_{k,r}^{(i)}
W_samples = array(0, c(n,V,R,N_s)) # Dictionary function(n=48,k=5,R=5,N_s=1000)
#-----------------------------------------------------------------------------#
...
#-----------------------------------------------------------------------------#
#G = matrix(rnorm(V*K),nr=V, nc=K) # G=
#Tau = rep(1,K)
#-----------------------------------------------------------------------------#
# W[,V,r] ~ MVN(0,c_w)
W = array(0, c(n,V,R))
...
#-----------------------------------------------------------------------------#
X = array(rnorm(V*R*N,mean=0,sd=1/sqrt(phi_x)),c(V,R,N)) # Page 3, equation 4 (Aninda)
#-----------------------------------------------------------------------------#
...
for (t in 1:burnin){
  ...
  for (i in 1:N){
    Omega_temp<-rpg.devroye(0.5*V*(V-1), n=1, lowerTriangle(Z + X[,,i] %*% t(X[,,i])))#Equation 2(Aninda)  
  ...
  # W_td = array(0,c(N,V,R))
  for (i in 1:N){
    W_td[i,,] = W[index_y[i],1,]  
  }
  
  for (i in 1:N){
    
    #-----------------------------------------------------------------------------#
    #Sample first row of X given the others (phi_x=1) Line303: Aninda change the transpose of W
    
    Sigma_v<-solve( phi_x * diag(R) + t(X[-1,,i]) %*% diag(Omega[-1,1,i],V-1,V-1) %*% X[-1,,i] )
    mu_v<-Sigma_v %*% (phi_x * diag(R) %*% W_td[i,1,] + (t(X[-1,,i]) %*% (A[-1,1,i] - 0.5 - Omega[-1,1,i] * Z[-1,1])))  
    ...
    #-----------------------------------------------------------------------------#
    #Sample other rows of X given the others (phi_x=1)
    for (v in 2:V){
      Sigma_v <- solve( phi_x * diag(R) + t(X[-v,,i]) %*% diag(Omega[-v,v,i],V-1,V-1) %*% X[-v,,i])
      mu_v<- Sigma_v %*% ( phi_x * diag(R) %*% W_td[i,v,] + t(X[-v,,i]) %*% (A[-v,v,i] - 0.5 - Omega[-v,v,i]*Z[-v,v]))  
      X[v,,i]<-mvrnorm(1,mu_v,Sigma_v)
      
      #mu_v<- Sigma_v %*% ( phi_x * t(W_td[i,1,]) %*% G[v,1] + t(X[-v,,i]) %*% (A[-v,v,i] - 0.5 - Omega[-v,v,i]*Z[-v,v]))  
      #X[v,,i]<-mvrnorm(1,mu_v,Sigma_v)
    }
    print(t)
    X_samples[,,,t] = X
  ...
  # mu_g = 0
  # for (i in 1:N){
  # mu_g = mu_g + W_td[i,1,] %*% X[u,,i]
  # }
  # mu_g = phi_x * mu_g
  # mu_g = Sigma_g %*% mu_g
  # G[u,1] = mvrnorm(1,mu_g,Sigma_g)
  # 
   #}
  ...
  #Sigma_w = solve(kronecker(diag(V), D) + kronecker(diag(V), inv_cov_W)) #Aninda
  
  Sigma_w = solve(D + inv_cov_W) #Aninda
  ...
  hat_X_mat = apply(hat_X,2,function(x){as.vector(t(x))}) # nV x R=3264 x 5
  
  for (r in 1:R){
    mu_w = Sigma_w %*%  t((phi_x * kronecker(diag(V)[1,],diag(n)))) %*% hat_X_mat[,r] #Aninda
    #mu_w = Sigma_w %*%  (phi_x * kronecker(t(G[,1]),diag(n)) )  %*% hat_X_mat[,r]
    W[,,r]= matrix(mvrnorm(1,mu_w,Sigma_w), nr=n, nc=1)   
  }
  
  W_samples[,,,t] = W #Aninda
}
...
for (t in 1:N_s){
  for (s in 1:thin){
    ...
    for (i in 1:N){
      ...
      #mu_v<-Sigma_v %*% ( phi_x * t(W_td[i,1,]) * G[1,1] + t(X[-1,,i]) %*% (A[-1,1,i] - 0.5 - Omega[-1,1,i] * Z[-1,1]) )  
      X[1,,i]<-mvrnorm(1,mu_v,Sigma_v)
    ...
    #kron_GG = phi_x * kronecker( (t(G) %*% G), D)
    #Sigma_w = solve(kron_inv_c_w + kron_GG) 
    #Sigma_w = solve(kronecker(diag(V), D) + kronecker(diag(V), inv_cov_W))
    Sigma_w = solve(D + inv_cov_W)
    epsilon <- 1e-6
    Sigma_w <- Sigma_w + epsilon * diag(ncol(Sigma_w))
    ...
    for (r in 1:R){
      mu_w = Sigma_w %*%  t((phi_x * kronecker(diag(n),diag(V)[1,]))) %*% hat_X_mat[,r]
      #mu_w = Sigma_w %*%  (phi_x * kronecker(t(G[,1]),diag(n)) )  %*% hat_X_mat[,r]
      W[,,r]= matrix(mvrnorm(1,mu_w,Sigma_w), nr=n, nc=1)   
    }
  }
...
mean_Pi_sub = array(0, c(V,V,N))# posterior mean of individual edge prob
...
W_mcmc <- as.mcmc(W_samples[,,,1:500])

# Generate trace plots for W
traceplot(W_mcmc)

# Convert the samples to an mcmc object
X_mcmc <- as.mcmc(X_samples[,,,1:100])

# Generate trace plots for Tau parameters
traceplot(X_mcmc)


# Convert the samples to an mcmc object
Z_mcmc <- as.mcmc(Z_samples[,,1])
